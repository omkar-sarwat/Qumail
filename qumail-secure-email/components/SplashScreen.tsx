...
const SplashScreen: React.FC<SplashScreenProps> = ({ onFinish }) => {
  ...
  useEffect(() => {
    // Sequence: 
    // 0-2200ms: Animation runs
    // 2200ms: Fade out starts
    ...
    >
      <div className="relative flex flex-col items-center">
        ...
        <div className="relative w-24 h-24 mb-8 flex items-center justify-center">
          {/* Orbital Ring 1 */}
          <div className="absolute inset-0 border border-indigo-100 rounded-full animate-[spin_3s_linear_infinite]" style={{ borderRadius: '40% 60% 70% 30% / 40% 50% 60% 50%' }}></div>
          
          {/* Orbital Ring 2 (Reverse & Offset) */}
          <div className="absolute inset-0 border border-indigo-200 rounded-full animate-[spin_4s_linear_infinite_reverse]" style={{ borderRadius: '60% 40% 30% 70% / 60% 30% 70% 40%' }}></div>
        ...
        <div className="text-center overflow-hidden">
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight mb-1 animate-in slide-in-from-bottom-4 fade-in duration-700 delay-300 fill-mode-backwards">
            QuMail
          </h1>
        ...
        <div className="absolute -bottom-16 w-48 h-1 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-indigo-600 w-1/2 rounded-full animate-[shimmer_1.5s_infinite] translate-x-[-100%]" 
                 style={{ 
