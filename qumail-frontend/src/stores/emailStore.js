...
const stripCssArtifacts = (text) => text
    .replace(CSS_MEDIA_QUERY, ' ')
    .replace(CSS_SELECTOR_BLOCK, ' ')
    .replace(/\/\*[\s\S]*?\*\//g, ' ');
const sanitizeSnippet = (rawSnippet, fallbackHtml, fallbackText) => {
    const source = rawSnippet || fallbackText || fallbackHtml || '';
    if (!source)
        return '';
    const cleaned = stripCssArtifacts(source
...
export const useEmailStore = create((set, get) => ({
    ...
    currentFilter: { folder: 'gmail_inbox' },
    isLoading: false,
    ...
    fetchEmails: async (filter = {}, reset = false) => {
        try {
            ...
            const newEmails = (response.emails || []).map((email) => ({
                id: email.id,
    ...
    fetchEmailDetails: async (emailId) => {
        try {
            ...
            set(state => ({
                emails: state.emails.map(e => e.id === emailId ? { ...e, ...email } : e),
                isLoadingEmail: false,
            }));
            return email;
        }
    ...
    markAsUnread: async (emailId) => {
        try {
            ...
            set(state => ({
                emails: state.emails.map(email => email.id === emailId ? { ...email, isRead: false } : email),
            }));
            get().updateUnreadCount();
