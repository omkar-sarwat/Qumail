...
export const NewComposeEmailModal: React.FC<NewComposeEmailModalProps> = ({
  ...
  const fetchDrafts = async () => {
    ...
    try {
      const response = await fetch('https://qumail-backend-8ttg.onrender.com/api/v1/emails/drafts', {
        headers: {
  ...
  const deleteDraft = async (draftId: string) => {
    ...
    try {
      const response = await fetch(`https://qumail-backend-8ttg.onrender.com/api/v1/emails/drafts/${draftId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
