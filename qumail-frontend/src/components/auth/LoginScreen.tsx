...
export const LoginScreen: React.FC = () => {
  ...
  const handleYahooLogin = async () => {
    ...
    try {
      console.log('Initiating Yahoo OAuth login...')
      
      // Get Yahoo OAuth URL from backend
      const response = await apiService.getYahooAuthUrl()
      console.log('Got Yahoo OAuth URL')
      
      // Store state in localStorage to prevent CSRF
      localStorage.setItem('yahoo_oauth_state', response.state)
      
      // Yahoo requires HTTPS redirect - redirect in browser
      // Note: For local dev, you may need to accept self-signed cert warning
      console.log('Redirecting to Yahoo OAuth...')
      window.location.href = response.authorization_url
      
    } catch (error: any) {
      console.error('Yahoo login failed:', error)
      const errorMsg = error.response?.data?.detail || error.message || 'Failed to start Yahoo authentication'
      setError(errorMsg)
              ...
              <Card className="bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl max-w-md mx-auto">
                <CardContent className="p-8">
                  ...
                  <Button
                    ...
                    disabled={isYahooLoading || isLoading}
                    className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-4 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center space-x-3"
                  >
                    {isYahooLoading ? (
                      <svg className="animate-spin h-5 w-5" viewBox="0 0 24 24">
                        <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-25" fill="none" />
                      ...
                      <>
                        {/* Yahoo Logo */}
                        <svg className="w-5 h-5" viewBox="0 0 24 24">
                          <path fill="currentColor" d="M12.996 4.001h3.477l-3.036 5.476 3.045 5.477h-3.486l-1.504-2.988-1.504 2.988H6.527l3.045-5.477L6.527 4.001h3.487l1.491 2.988zm-.009 12.954h2.006v3.041h-2.006z"/>
                        </svg>
