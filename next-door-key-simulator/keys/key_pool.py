...
class KeyPool:
    ...
    def get_key(self, key_size: int, timeout: Optional[float] = None, remove: bool = False) -> Optional[Dict[str, str]]:
        ...
        # Convert default key size from bytes to bits for comparison
        default_key_size_bits = self.default_key_size * 8
        
        with self.condition:
            # Check if non-default size is requested
            if key_size and key_size != default_key_size_bits:
                ...
                # Leave pool untouched for non-default request
            ...
            if remove:
                # OTP consumption: remove key permanently
                key = self.keys.pop()
                print(f'INFO: Removing key from pool for OTP consumption ({len(self.keys)}/{self.max_key_count})')
            else:
                # enc_keys: copy key without removing (will be removed during dec_keys)
                key = self.keys[0].copy()
                print(f'INFO: Copying key from pool for enc_keys (NOT removed yet, {len(self.keys)}/{self.max_key_count})')
            return key
